...
BinPath = 'MachineLearning-Portfolio/3-Classification/5-Naive_Bayes/bin'
...
TrainingPlot <- plot(set[, -3],
     main = 'Naive Bayesian Classification (Training set)',
     xlab = 'Age', ylab = 'Estimated Salary',
     xlim = range(X1), ylim = range(X2))
contour(X1, X2, matrix(as.numeric(y_grid), length(X1), length(X2)), add = TRUE)
points(grid_set, pch = '.', col = ifelse(y_grid == 1, 'springgreen3', 'tomato'))
points(set, pch = 21, bg = ifelse(set[, 3] == 1, 'green4', 'red3'))
ggsave(file="NaiveBayes_Training_R.png", plot=TrainingPlot, path=BinPath)
...
TestingPlot <- plot(set[, -3],
     main = 'Naive Bayesian Classification (Testing set)',
     xlab = 'Age', ylab = 'Estimated Salary',
     xlim = range(X1), ylim = range(X2))
contour(X1, X2, matrix(as.numeric(y_grid), length(X1), length(X2)), add = TRUE)
points(grid_set, pch = '.', col = ifelse(y_grid == 1, 'springgreen3', 'tomato'))
points(set, pch = 21, bg = ifelse(set[, 3] == 1, 'green4', 'red3'))
ggsave(file="NaiveBayes_Testing_R.png", plot=TestingPlot, path = BinPath)