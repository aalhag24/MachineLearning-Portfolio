...
BinPath = 'MachineLearning-Portfolio/3-Classification/3-Support_Vector_Machine/bin'
...
ggsave("LinearSVM_Training_R.png",  path = BinPath)
...
y_grid = predict(classifier, type = 'response', newdata = grid_set)
plot(set[, -3],
     main = 'SVM (Testing set)',
     xlab = 'Age', ylab = 'Estimated Salary',
     xlim = range(X1), ylim = range(X2))
contour(X1, X2, matrix(as.numeric(y_grid), length(X1), length(X2)), add = TRUE)
points(grid_set, pch = '.', col = ifelse(y_grid == 1, 'springgreen3', 'tomato'))
points(set, pch = 21, bg = ifelse(set[, 3] == 1, 'green4', 'red3'))
ggsave("LinearSVM_Testing_R.png",  path = BinPath)