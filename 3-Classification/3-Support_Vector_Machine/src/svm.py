...
TrainingPlot.savefig(BinPath + '\LinearSVM_Training_PY.png')
...
for i,j in enumerate(np.unique(Y_set)):
    plt.scatter(X_set[Y_set == j,0], X_set[Y_set == j,1],
                c = ListedColormap(('darkred', 'darkgreen'))(i), 
                label = j)
plt.title('Linear Support Vector Machine (Testing set)')
plt.xlabel('Age')
plt.ylabel('Estimated Salary')
plt.legend()
plt.show()

TestingPlot.savefig(BinPath + '\LinearSVM_Testing_PY.png')